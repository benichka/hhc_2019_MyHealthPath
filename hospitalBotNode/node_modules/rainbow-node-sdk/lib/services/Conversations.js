...
class Conversations {
    ...
    async sendCorrectedChatMessage(conversation, data, origMsgId) {
        ...
        if (originalMessageFrom !== that._rest.loggedInUser.jid_im) {
            ...
            throw new Error("(sendCorrectedChatMessage) forbidden Action - only sent messages can be modified");
        }
        ...
        if (lastEditableMsg.id !== originalMessage.id) {
            ...
            throw new Error("(sendCorrectedChatMessage) forbidden Action - only last sent message can be modified");
        }
        ...
        try {
            ...
            that._logger.log("error", LOG_ID + "createFileDescriptor error");
            let error = new Error("(sendCorrectedChatMessage) error while sending corrected message : " + err );
...
/*
    ...
    getConversationById(conversationId) {
        let that = this;
        that._logger.log("internal", LOG_ID + " (getConversationById) conversationId : ", conversationId);
    ...
    openConversationForContact (contact) {
        ...
        return new Promise(function (resolve, __reject) {
            ...
            if (!contact) {
                ...
                that
                    ._logger
                    .log("info", LOG_ID + " :: Try to create of get a conversation with " + contact.lastName + " " + contact.firstName);
                ...
                that.getOrCreateOneToOneConversation(contact.jid)
                    .then(function (conversation) {
                        that
                            ._logger
                            .log("info", LOG_ID + "  :: Conversation retrieved or created " + conversation.id);
                        resolve(conversation);
                    }).catch(function (result) {
                    that
                        ._logger
                        .log("info", LOG_ID + "  :: Error");
                    __reject(result);
                });
            }
        });
    ...
    onRoomAdminMessageEvent(__event, roomJid, userJid, type, msgId) {
        this
            ._logger
            .log("info", LOG_ID + " onRoomAdminMessageEvent");
    ...
    reinit() {
        ...
        return new Promise((resolve) => {
            ...
            that
                ._logger
                .log("info", LOG_ID + " Re-initialize conversation service");
