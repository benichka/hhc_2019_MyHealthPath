...
const ReceiptNS = "urn:xmpp:receipts";
        ...
        if (that.useXMPP) {
            ...
            return new Promise((resolve, reject) => {
                that
                    ...
                    resolve({from: this.jid_im, to: jid, type: "groupchat", id: id, date: new Date(), message: message, content: content, subject: subject, lang: lang});
                }).catch((err) => {
                    reject(err);
                });
            });
...
//        $log.info("[Conversation] >sendCorrectedChatMessage: origMsgId=" + origMsgId)
        ...
        if (conversation.type === Conversation.Type.ONE_TO_ONE) {
            ...
            xmppMessage = xml("message", { to: to, type: "chat", id: messageToSendID, "xml:lang": lang },
                ...
                xml("replace", { id: origMsgId, "xmlns": "urn:xmpp:message-correct:0" }),
                xml("store", { "xmlns": "urn:xmpp:hints" }),
                xml("request", { "xmlns": ReceiptNS }),
                xml("active", { "xmlns": ChatstatesNS })
            );
        }
        ...
        if (this.useXMPP) {
            ...
            return new Promise((resolve, reject) => {
                    that
                        ...
                        that.logger.log("error", LOG_ID + "(markMessageAsRead) error : ", error);
                        reject(err);
                    });
                });
        }
    ...
    voiceMessageQuery(jid) {
        ...
        const CALLSERVICE_NS = "urn:xmpp:pbxagent:callservice:1";

        let nsAttr = {xmlns: CALLSERVICE_NS};
