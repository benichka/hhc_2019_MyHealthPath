...
//     * @param {uploadAFileByChunk~progressCallback} progressCallback [required] initial size of whole file to be sent before partition
    ...
    uploadAFileByChunk(fileDescriptor, filePath /*, progressCallback */) {
        ...
        let range = that.capabilities.maxChunkSizeUpload;
    ...
    getBlobFromUrlWithOptimization(url, mime, fileSize, fileName, uploadedDate ) {
        ...
        if (uploadedDate.length !== 0) {
            url += "?update=" + MD5.hexdigest(uploadedDate);
        }

        if (!!that.capabilities.maxChunkSizeDownload && fileSize !== 0 && fileSize > that.capabilities.maxChunkSizeDownload) {
            return new Promise((resolve, reject) => {
                let range = that.capabilities.maxChunkSizeDownload;
                ...
                Promise.all(promiseArray)
                    .then(
                        ...
                        (errorResponse) => {
                            let errorMessage = "[FileServerService] getBlobFromUrlWithOptimization failure : " + error.message;
